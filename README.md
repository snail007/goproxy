...
1. [HTTP代理](#1HTTP代理)
    ...
    2. [普通二级HTTP代理](#12普通二级HTTP代理)
    3. [HTTP二级代理(加密)](#13HTTP二级代理(加密))
    4. [HTTP三级代理(加密)](#14HTTP三级代理(加密))
    5. [Basic认证](#15Basic认证)
    6. [强制走上级HTTP代理](#16HTTP代理流量强制走上级HTTP代理)
    7. [通过SSH中转](#17HTTP(S)通过SSH中转)
        1. [用户名和密码的方式](#171ssh用户名和密码的方式)
        2. [用户名和密钥的方式](#172ssh用户名和密钥的方式)
    8. [查看帮助](18查看帮助)
