...
func StartWithLog(serviceID, serviceArgsStr string, loggerCallback LogCallback) (errStr string) {
	...
	httpArgs.ProxyInternalIp = http.Flag("proxy-internal-ip", "force internal ip use parent proxy").Default("false").Bool()
	...
	socksArgs.ProxyInternalIp = socks.Flag("proxy-internal-ip", "force internal ip use parent proxy").Default("false").Bool()
